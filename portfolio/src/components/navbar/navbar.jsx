      ...
      <div className="w-full h-[100px] relative flex items-center justify-between px-6 lg:px-16">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-white text-4xl font-bold tracking-wider relative ">
            SABARI
            <span className="text-red-500 animate-pulse absolute -top-2 -right-4 text-5xl">
              .
            </span>
          </span>
        ...
        <div className="hidden lg:flex items-center space-x-12">
          ...
          <a
            ...
            className="px-6 py-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors duration-300 transform hover:scale-105"
          >
            Resume
          </a>
        </div>
        ...
        <div className="flex flex-col items-center py-8 space-y-6">
          {menuItems.map((item) => (
            <button
              key={item}
              onClick={() =>
                handleSmoothScroll(item.toLowerCase(), setIsOpen(false))
              }
